...
export default function CourseBrowserPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => courseService.list({ q: dq, category, level, isFree: isFree || undefined, sort, page, limit: 12 }),
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-10">
      <h1 className="font-display text-3xl font-bold text-gray-900 mb-2">All Courses</h1>
      <p className="text-gray-500 mb-8">Explore our full catalogue of expert-led courses</p>
      ...
      <div className="flex flex-wrap gap-3 mb-8">
        ...
        <select value={level} onChange={e => { setLevel(e.target.value); setPage(1); }} className="input w-auto">
      ...
      {isLoading ? <PageLoader /> : courses.length === 0 ? (
        <EmptyState icon={Search} title="No courses found" description="Try adjusting your filters or search term." />
      ) : (
        <>
          <p className="text-sm text-gray-500 mb-4">{data?.data?.data?.total || 0} courses found</p>
